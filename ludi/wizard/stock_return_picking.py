...
class LudiStockReturnPicking(models.TransientModel):
    ...
    def _create_returns(self):
        ...
        date_to_return = self.picking_id.date_to_return
        # create new picking for returned products
        picking_type_id = self.picking_id.picking_type_id.return_picking_type_id.id or self.picking_id.picking_type_id.id
        return_products = datetime(date_to_return.year,date_to_return.month,date_to_return.day,8,0,0,0).astimezone(pytz.timezone("America/Mexico_City"))
        new_picking = self.picking_id.copy({
            'move_lines': [],
            'picking_type_id': picking_type_id,
            'state': 'draft',
            'origin': _("Return of %s", self.picking_id.name),
            'scheduled_date': return_products if self.picking_id.return_products == True else self.picking_id.scheduled_date,
            'stock_picking_return': self.picking_id.id if self.picking_id.return_products == True else None,
        ...
        new_picking.update({
            'scheduled_date': self.picking_id.date_to_return if self.picking_id.return_products == True else self.picking_id.scheduled_date,
        })
